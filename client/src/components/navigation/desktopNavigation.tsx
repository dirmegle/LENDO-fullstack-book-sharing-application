...
export default function Navigation() {
  ...
  return (
    <aside className={navClasses}>
      <button onClick={handleClick} className='absolute top-0 bg-accent-green w-full rounded-t-md flex justify-center p-1 hover:bg-muted-green transition-all duration-300 ease-in-out border-b border-border'>{isExpanded ? <ArrowLeft /> : <ArrowRight />}</button>
      <nav>{navLinks}</nav>
    </aside>
  )
}
