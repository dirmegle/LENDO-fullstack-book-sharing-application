...
export default function BooksDisplay({ bookCopies, link, title }: BooksDisplayProps) {
  ...
  return (
    <div>
      <div onClick={() => navigate(link)} className="flex flex-row justify-between">
        <h2 className="font-medium text-3xl mb-4">{title}</h2>
        <Button variant="outline">See all</Button>
      </div>
      <div className="lg:overflow-hidden overflow-y-auto  lg:h-max-content h-[275px] border border-border p-2 shadow-[3px_3px_#141414]">
